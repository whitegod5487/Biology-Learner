...
## v1.0.0（2026-08-21）
**更新主題**：題庫全面重新生成 — 每章 100 題（中英雙語）

- **中文題庫重生成**：根據「新高中基礎生物學試題庫」資料庫，將全部 37 章題庫重新生成為**每章 100 題**：
  - [`questionBank1.js`](questionBank1.js)：課題 1–13，共 **1,300 題**。
  - [`questionBank2.js`](questionBank2.js)：課題 14–25，共 **1,200 題**。
  - [`questionBank3.js`](questionBank3.js)：課題 26–37，共 **1,200 題**。
  - 合共 **3,700 題**，全部經格式驗證（每題 4 選項、4 原因、`reasons[correct] = "正確答案"`）。
- **英文題庫重生成**（English UK）：同步將英文題庫擴充至每章 100 題：
  - [`questionBank1.en.js`](questionBank1.en.js)：課題 1–13，共 **1,300 題**。
  - [`questionBank2.en.js`](questionBank2.en.js)：課題 14–25，共 **1,200 題**。
  - [`questionBank3.en.js`](questionBank3.en.js)：課題 26–37，共 **1,200 題**。
  - 合共 **3,700 題**，全部經格式驗證（`reasons[correct] = "Correct answer"`）。
- **練習模式全數可用**：[`app.js`](app.js) 的 `startPractice()` 移除「每課題 20 題」上限（`qs.slice(0, 20)`），改為載入整章 **100 題**；課本模式維持整冊練習。
- **任務線計分更新**：課題練習「全對」條件由 20/20 改為 **100/100**（+25 分，每課題一次）；同步更新 [`app.js`](app.js) 的任務線註解與分數規則字串（中英）、[`faq.js`](faq.js) 的「如何獲得分數」答案、[`index.html`](index.html) 練習頁副標。
- 版本更新至 **v1.0.0**，全部 `<script>` 快取參數更新為 `?v=1.0.0`。

## v0.9.10（2026-08-21）
**更新主題**：移除「多項選擇題」頁面的「DSE 試卷」按鈕

- 依用戶意見，刪除「多項選擇題」頁（`mcPage`）中的「🗞️ DSE 試卷」卡片按鈕，令該頁只保留「練習模式」與「挑戰模式」兩個入口；「DSE 試卷」入口仍保留於主頁。
- [`index.html`](index.html) 移除 `mcPage` 內的 DSE 試卷卡片，無需改動 [`app.js`](app.js) 的 `goToDsePage()` 函式（主頁仍會用到）。
- 版本更新至 **v0.9.10**，全部 `<script>` 快取參數更新為 `?v=0.9.10`。
